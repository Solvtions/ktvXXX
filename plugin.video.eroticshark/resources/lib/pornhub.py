...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\pornhub.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/pornhub.py'
...
def main_directorys():
    url = build_url({'mode': 'pornhub', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'pornhub', 'func': 'category_directorys', 'foldername': 'Categories'})
...
def category_directorys():
    top_url = 'http://www.pornhub.com/categories'
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    category_links = re.compile('class="category-wrapper">\s*<a\s*href="([^"]+)"\s*alt="([^"]+)"').findall(url_content)

    if category_links:
        for cic in range(0, len(category_links)):
            url = build_url({'mode': 'pornhub', 'func': 'video_links', 'foldername': category_links[cic][0], 'pagenum': '0'})
...
def video_links(foldername, pagenum):
    ...
    if re.compile('(/search)').findall(foldername):
        top_url = 'http://www.pornhub.com' + foldername + '&page=' + pagenum
    elif re.compile('(/video\?c)').findall(foldername):
        top_url = 'http://www.pornhub.com' + foldername + '&page=' + pagenum
    else:
        top_url = 'http://www.pornhub.com' + foldername + '?page=' + pagenum
    ...
    porn_links = re.compile('href="/view_video\.php\?viewkey=([^"]+)"\s*title="([^"]+)').findall(url_content)
    porn_images = re.compile('mb="([^"]+)').findall(url_content)
    next_page = re.compile('(_next)').findall(url_content)


    for cic in range(0, len(porn_links), 2):
        url = build_url({'mode': 'pornhub', 'func': 'find_videourls', 'foldername': porn_links[cic][0], 'title': porn_links[cic][1], 'folderimage': porn_images[cic/2]})
        download_args = 'DOWNLOAD, ' + porn_links[cic][0] + ',' + porn_links[cic][1].decode('utf-8')
        li = xbmcgui.ListItem(porn_links[cic][1], iconImage=porn_images[cic/2])
    ...
    if next_page:
        url = build_url({'mode': 'pornhub', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
...
def search_links():
    ...
    top_url = '/video/search?search=' + urllib.quote(search_text, '')
...
def find_videourls(foldername, foldertitle, folderimage, isdownload):
    ...
    direct_url = re.compile('player_quality_[0-9a-zA-Z]+\s*=\s*\'([^\']+)').findall(url_content)
