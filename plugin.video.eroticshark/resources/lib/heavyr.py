...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\heavyr.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/heavyr.py'
...
def main_directorys():
    url = build_url({'mode': 'heavyr', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'heavyr', 'func': 'category_directorys', 'foldername': 'Categories'})
    li = xbmcgui.ListItem('Categories', iconImage='DefaultFolder.png')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)
    
    video_links('/videos/recent/', '-1')

    return

def category_directorys():
    top_url = 'http://www.heavy-r.com/categories/'
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    category_links = re.compile('f="(\/porn_videos\/[^"]+)"\s*title="([^"]+)').findall(url_content)

    if category_links:
        for cic in range(0, len(category_links)):
            url = build_url({'mode': 'heavyr', 'func': 'video_links', 'foldername': category_links[cic][0], 'pagenum': '-1'})
...
def video_links(foldername, pagenum):
    ...
    if re.compile('(\/search\/)').findall(foldername):
        top_url = 'http://www.heavy-r.com' + foldername + pagenum + '.html'
        max_page = 20
    elif re.compile('(\/videos\/)').findall(foldername):
        top_url = 'http://www.heavy-r.com' + foldername + pagenum + '/'
        max_page = 40
    else:
        top_url = 'http://www.heavy-r.com' + foldername + '/' + pagenum
        max_page = 32
    ...
    porn_links = re.compile('video-item[^\/]+(\/[^"]+)"[^<]+<[^"]+"([^"]+)').findall(url_content)
    porn_title = re.compile('<h4.class="title"><[^>]+>([^<]+)').findall(url_content)
    ...
    if no_page:
        ...
        for cic in range(0, len(porn_title)):
            url = build_url({'mode': 'heavyr', 'func': 'find_videourls', 'foldername': porn_links[cic][0], 'title': porn_title[cic], 'folderimage': porn_links[cic][1]})
            download_args = 'DOWNLOAD, ' + porn_links[cic][0] + ',' + porn_title[cic].decode('utf-8')
            li = xbmcgui.ListItem(porn_title[cic], iconImage=porn_links[cic][1])
        ...
        if next_page:
            url = build_url({'mode': 'heavyr', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
...
def search_links():
    ...
    top_url = '/search/' + urllib.quote(search_text, '') + '_'

    video_links(top_url, '-1')

    return

def find_videourls(foldername, foldertitle, folderimage, isdownload):
    top_url = 'http://www.heavy-r.com' + foldername
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    direct_url = re.compile('src\s*:\s*\'([^\']+)').findall(url_content)
