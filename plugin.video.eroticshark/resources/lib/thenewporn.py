...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\thenewporn.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/thenewporn.py'
...
def main_directorys():
    url = build_url({'mode': 'thenewporn', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'thenewporn', 'func': 'category_directorys', 'foldername': 'Categories'})
...
def category_directorys():
    top_url = 'http://www.thenewporn.com/categories/'
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    category_links = re.compile('href="http://www\.thenewporn\.com(/categories/[^"]+)".*\s*.*\s*<[^<]+<[^<]+<figcaption>\s*([a-zA-Z0-9 ]+)').findall(url_content)

    if category_links:
        for cic in range(0, len(category_links)):
            url = build_url({'mode': 'thenewporn', 'func': 'video_links', 'foldername': category_links[cic][0], 'pagenum': '0'})
...
def video_links(foldername, pagenum):
    ...
    if re.compile('(/search/)').findall(foldername):
        top_url = 'http://www.thenewporn.com' + foldername[:8] + pagenum + '/' + foldername[8:]
    else:
        top_url = 'http://www.thenewporn.com' + foldername + pagenum + '/'

    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    porn_links = re.compile('thenewporn\.com(/videos/[^"]+).+?title="([^"]+)"[^h]+([^"]+)').findall(url_content)
    
    if (len(porn_links) == 70):
        next_page = True
    else:
        next_page = False
    ...
    if porn_links:
        for cic in range(len(porn_links)):
            url = build_url({'mode': 'thenewporn', 'func': 'find_videourls', 'foldername': porn_links[cic][0], 'title': porn_links[cic][1], 'folderimage': porn_links[cic][2]})
            download_args = 'DOWNLOAD, ' + porn_links[cic][0] + ',' + porn_links[cic][1].decode('utf-8')
            li = xbmcgui.ListItem(porn_links[cic][1], iconImage=porn_links[cic][2])
        ...
        if next_page:
            url = build_url({'mode': 'thenewporn', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
...
def find_videourls(foldername, foldertitle, folderimage, isdownload):
    top_url = 'http://www.thenewporn.com/' + foldername
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    direct_url = re.compile('video_url:[^\']+\'([^\']+)').findall(url_content)
