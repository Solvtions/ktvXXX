...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\pornicom.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/pornicom.py'
...
def main_directorys():
    url = build_url({'mode': 'pornicom', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'pornicom', 'func': 'category_directorys', 'foldername': 'Categories'})
...
def category_directorys():
    top_url = 'http://pornicom.com/categories/'
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    category_links = re.compile('f="http://pornicom\.com(/categories[^"]+)"[^>]+>([^<]+)').findall(url_content)

    if category_links:
        for cic in range(8, len(category_links), 2):
            url = build_url({'mode': 'pornicom', 'func': 'video_links', 'foldername': category_links[cic][0], 'pagenum': '0'})
...
def video_links(foldername, pagenum):
    ...
    if re.compile('(/search)').findall(foldername):
        top_url = 'http://www.pornicom.com' + foldername[:8] + pagenum + foldername[8:]
    else:
        top_url = 'http://www.pornicom.com' + foldername + pagenum + '/'

    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return
    
    porn_links = re.compile('class="image"[^"]+"url"\s+href="([^"]+)".+title="([^"]+)"[^:]+://([^"]+)').findall(url_content)
    no_page = re.compile('(There are no videos in the list)').findall(url_content)

    if len(porn_links) == 36:
        next_page = True
    else:
        next_page = False
    ...
    if no_page:
        ...
        for cic in range(0, len(porn_links)):
            url = build_url({'mode': 'pornicom', 'func': 'find_videourls', 'foldername': porn_links[cic][0], 'title': porn_links[cic][1], 'folderimage': 'http://' + porn_links[cic][2]})
        ...
        if next_page:
            url = build_url({'mode': 'pornicom', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
...
def find_videourls(foldername, foldertitle, folderimage, isdownload):
    ...
    direct_url = re.compile('\[\'video_html5_url\'\]=\'([^\']+)').findall(url_content)
