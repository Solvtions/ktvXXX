...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\wankoz.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/wankoz.py'
...
def main_directorys():
    url = build_url({'mode': 'wankoz', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'wankoz', 'func': 'category_directorys', 'foldername': 'Categories'})
...
def category_directorys():
    top_url = 'http://www.wankoz.com/categories/'
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    category_links = re.compile('div.class="item"[^"]+"http://www.wankoz.com([^"]+)"[^>]+>([^<]+)').findall(url_content)

    if category_links:
        for cic in range(0, len(category_links)):
            url = build_url({'mode': 'wankoz', 'func': 'video_links', 'foldername': category_links[cic][0], 'pagenum': '0'})
...
def video_links(foldername, pagenum):
    ...
    if re.compile('(/search)').findall(foldername):
        top_url = 'http://www.wankoz.com' + foldername[:8] + pagenum + foldername[8:]
        max_item = 30
    else:
        top_url = 'http://www.wankoz.com' + foldername + pagenum + '/'
        max_item = 1
    ...
    if no_page:
        ...
        for cic in range(0, len(porn_links)):
            url = build_url({'mode': 'wankoz', 'func': 'find_videourls', 'foldername': porn_links[cic][0], 'title': porn_links[cic][1], 'folderimage': 'http://' + porn_links[cic][2]})
        ...
        if next_page:
            url = build_url({'mode': 'wankoz', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
