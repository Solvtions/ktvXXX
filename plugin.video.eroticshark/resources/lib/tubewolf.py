...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\tubewolf.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/tubewolf.py'
...
def main_directorys():
    url = build_url({'mode': 'tubewolf', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'tubewolf', 'func': 'category_directorys', 'foldername': 'Categories'})
    li = xbmcgui.ListItem('Categories', iconImage='DefaultFolder.png')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    video_links('/latest-updates/', '0')

    return

def category_directorys():
    top_url = 'http://www.tubewolf.com/categories/'
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    category_links = re.compile('<li><a\s*href="http://www\.tubewolf\.com(/categories/[^"]+)"\s*title="([^"]+)').findall(url_content)

    if category_links:
        for cic in range(0, len(category_links)):
            url = build_url({'mode': 'tubewolf', 'func': 'video_links', 'foldername': category_links[cic][0], 'pagenum': '0'})
...
def video_links(foldername, pagenum):
    ...
    if re.compile('(/search)').findall(foldername):
        top_url = 'http://www.tubewolf.com' + foldername[:8] + pagenum +'/' + foldername[8:]
    else:
        top_url = 'http://www.tubewolf.com' + foldername + pagenum +'/'

    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    porn_links = re.compile('m(/movies/[^"]+)"[^"]+"[^"]+"[^"]+"([^"]+)"[^:]+://([^"]+)').findall(url_content)
    next_page = re.compile('(btn-next)').findall(url_content)

    if porn_links:
        for cic in range(len(porn_links)):
            url = build_url({'mode': 'tubewolf', 'func': 'find_videourls', 'foldername': porn_links[cic][0], 'title': porn_links[cic][1], 'folderimage': 'http://' + porn_links[cic][2]})
        ...
        if next_page:
            url = build_url({'mode': 'tubewolf', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
...
def find_videourls(foldername, foldertitle, folderimage, isdownload):
    top_url = 'http://www.tubewolf.com' + foldername
