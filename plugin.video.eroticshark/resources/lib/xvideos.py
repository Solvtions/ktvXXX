...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\xvideos.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/xvideos.py' 
...
def main_directorys():
    url = build_url({'mode': 'xvideos', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'xvideos', 'func': 'category_directorys', 'foldername': 'Categories'})
...
def category_directorys():
    ...
    category_links = re.compile('href="(/c/[^"]+)"[^>]+>([^<]+)<').findall(url_content)

    if category_links:
        for cic in range(1, len(category_links)):
            url = build_url({'mode': 'xvideos', 'func': 'video_links', 'foldername': category_links[cic][0], 'pagenum': '0'})
...
def video_links(foldername, pagenum):
    which_url = re.compile('(/?k=)').findall(foldername)
    cat_url = re.compile('(/c/)').findall(foldername)

    if which_url:
        top_url = 'http://www.xvideos.com' + foldername + '&p=' + pagenum
    elif cat_url:
        top_url = 'http://www.xvideos.com' + foldername[:3] + pagenum + '/' + foldername[3:]
    else:
        top_url = 'http://www.xvideos.com' + foldername + pagenum + '/'
    ...
    next_page = re.compile('(Next)').findall(url_content)

    if porn_links:
        for cic in range(0, len(porn_links), 2):
            url = build_url({'mode': 'xvideos', 'func': 'find_videourls', 'foldername': porn_links[cic][0], 'title': porn_links[cic+1][1], 'folderimage': porn_links[cic][1]})
            download_args = 'DOWNLOAD, ' + porn_links[cic][0] + ',' + porn_links[cic+1][1].decode('utf-8')
        ...
        if next_page:
            url = build_url({'mode': 'xvideos', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
...
def search_links():
    ...
    top_url = '/?k=' + urllib.quote(search_text, '')
