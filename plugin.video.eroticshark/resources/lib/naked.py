...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\naked.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/naked.py'
...
def main_directorys():
    url = build_url({'mode': 'naked', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'naked', 'func': 'category_directorys', 'foldername': 'Categories'})
...
def category_directorys():
    top_url = 'http://nakedtube.com/channels/'
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    category_links = re.compile('pic-name">([^<]+)[^"]+"(/channels/[^/]+/)').findall(url_content)

    if category_links:
        for cic in range(1, len(category_links)):
            url = build_url({'mode': 'naked', 'func': 'video_links', 'foldername': category_links[cic-1][1], 'pagenum': '0'})
            li = xbmcgui.ListItem(category_links[cic][0].decode('utf-8').strip(), iconImage='DefaultFolder.png')
            xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                        listitem=li, isFolder=True)
...
def video_links(foldername, pagenum):
    ...
    pagenum += 1
    pagenum = str(pagenum)
    top_url = 'http://nakedtube.com' + foldername + 'page' + pagenum + '.html'

    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    porn_links = re.compile('href="(/video/[^"]+)"[^<]+<[^<]+<.+alt="([^"]+)"[^"]+"([^"]+)').findall(url_content)
    ...
    if no_page:
        ...
        for cic in range(len(porn_links)):
            url = build_url({'mode': 'naked', 'func': 'find_videourls', 'foldername': porn_links[cic][0], 'title': porn_links[cic][1], 'folderimage': porn_links[cic][2]})
        ...
        if next_page:
            url = build_url({'mode': 'naked', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
...
def find_videourls(foldername, foldertitle, folderimage, isdownload):
    top_url = 'http://nakedtube.com' + foldername
