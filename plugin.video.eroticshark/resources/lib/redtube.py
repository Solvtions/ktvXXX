...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\redtube.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/redtube.py'
...
def main_directorys():
    url = build_url({'mode': 'redtube', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'redtube', 'func': 'category_directorys', 'foldername': 'Categories'})
...
def category_directorys():
    top_url = 'http://www.redtube.com/categories'
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    category_links = re.compile('categoryTitle">\s*<a\s*title="([^"]+)"\s*href="([^"]+)').findall(url_content)

    if category_links:
        for cic in range(0, len(category_links)):
            url = build_url({'mode': 'redtube', 'func': 'video_links', 'foldername': category_links[cic][1], 'pagenum': '0'})
            li = xbmcgui.ListItem(category_links[cic][0].decode('utf-8'), iconImage='DefaultFolder.png')
            xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                        listitem=li, isFolder=True)
...
def video_links(foldername, pagenum):
    ...
    if re.compile('(/?search)').findall(foldername):
        top_url = 'http://www.redtube.com' + foldername + '&page=' + pagenum
    else:
        top_url = 'http://www.redtube.com' + foldername + '?page=' + pagenum
    ...
    porn_links = re.compile('<img title="([^"]+)" id="([0-9]+)".+data-src="([^"]+)').findall(url_content)
    
    if len(porn_links) >= 24:
        next_page = True
    else:
        next_page = False

    for cic in range(0, len(porn_links)):
        url = build_url({'mode': 'redtube', 'func': 'find_videourls', 'foldername': porn_links[cic][1], 'title': porn_links[cic][0], 'folderimage': porn_links[cic][2]})
        download_args = 'DOWNLOAD, ' + porn_links[cic][1] + ',' + porn_links[cic][0].decode('utf-8')
        li = xbmcgui.ListItem(porn_links[cic][0], iconImage=porn_links[cic][2])
    ...
    if next_page:
        url = build_url({'mode': 'redtube', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
...
def find_videourls(foldername, foldertitle, folderimage, isdownload):
    top_url = 'http://www.redtube.com/' + foldername
