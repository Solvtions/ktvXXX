...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\eporner.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/eporner.py'
...
def main_directorys():
    url = build_url({'mode': 'eporner', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'eporner', 'func': 'category_directorys', 'foldername': 'Categories'})
...
def category_directorys():
    top_url = 'http://www.eporner.com/categories/'
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    category_links = re.compile('f="(/category/[^"]+)"\s*title="([^"]+)').findall(url_content)

    if category_links:
        for cic in range(0, len(category_links)):
            url = build_url({'mode': 'eporner', 'func': 'video_links', 'foldername': category_links[cic][0], 'pagenum': '0'})
...
def video_links(foldername, pagenum):
    top_url = 'http://www.eporner.com' + foldername + pagenum + '/'

    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    porn_links = re.compile('href="(/hd-porn/[^"]+)"[^"]+"([^"]+)[^:]+://([^"]+)').findall(url_content)
    next_page = re.compile('(Next)').findall(url_content)

    for cic in range(0, len(porn_links)):
        url = build_url({'mode': 'eporner', 'func': 'find_videourls', 'foldername': porn_links[cic][0], 'title': porn_links[cic][1], 'folderimage': 'http://' + porn_links[cic][2]})
    ...
    if next_page:
        url = build_url({'mode': 'eporner', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
