...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\tube8.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/tube8.py'
...
def main_directorys():
    url = build_url({'mode': 'tube8', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'tube8', 'func': 'category_directorys', 'foldername': 'Categories'})
    li = xbmcgui.ListItem('Categories', iconImage='DefaultFolder.png')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    video_links('/latest/', '0')

    return

def category_directorys():
    top_url = 'http://www.tube8.com/categories.html'
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    category_links = re.compile('f="http://www.tube8.com(/cat/[^"]+)">([^<]+)').findall(url_content)

    if category_links:
        for cic in range(0, len(category_links) / 2):
            url = build_url({'mode': 'tube8', 'func': 'video_links', 'foldername': category_links[cic][0], 'pagenum': '0'})
...
def video_links(foldername, pagenum):
    ...
    if re.compile('(/search)').findall(foldername):
        ...
        base_compile = 'class="video_title">\s<a\shref="(http://www\.tube8\.com[^"]+)"\stitle="([^"]+)"'
    else:
        top_url = 'http://www.tube8.com' + foldername + 'page/' + pagenum + '/'
        base_compile = 'class="video_title">\s<a\shref="(http://www\.tube8\.com[^"]+)"\stitle="([^"]+)"'
    ...
    porn_links = re.compile(base_compile).findall(url_content)
    porn_images = re.compile('\.image\.tube8\.phncdn\.com/(.+jpg)').findall(url_content)
    next_page = re.compile('(pagination_next)').findall(url_content)
    no_page = re.compile('(warning-noresult")').findall(url_content)

    if no_page:
        no_video('')
    else:
        for cic in range(len(porn_links)):
            url = build_url({'mode': 'tube8', 'func': 'find_videourls', 'foldername': porn_links[cic][0], 'title': porn_links[cic][1],
                            'folderimage': 'http://cdn1.image.tube8.phncdn.com/' + porn_images[cic]})

            download_args = 'DOWNLOAD, ' + porn_links[cic][0] + ',' + porn_links[cic][1].decode('utf-8')
            li = xbmcgui.ListItem(porn_links[cic][1], iconImage='http://cdn1.image.tube8.phncdn.com/' + porn_images[cic])
        ...
        if next_page:
            url = build_url({'mode': 'tube8', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
...
def search_links():
    ...
    top_url = '/searches.html?q=' + urllib.quote(search_text, '')
...
def find_videourls(foldername, foldertitle, folderimage, isdownload):
    ...
    direct_url = re.compile('videoUrlJ[^"]+"([^"]+)').findall(url_content)
