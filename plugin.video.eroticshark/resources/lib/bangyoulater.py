...
if sys.platform == 'win32':
    ...
    download_script = thisAddonDir + '\\resources\\lib\\bangyoulater.py'
else:
    imageDir = thisAddonDir + '/resources/image/'
    download_script = thisAddonDir + '/resources/lib/bangyoulater.py'
...
def main_directorys():
    url = build_url({'mode': 'bangyoulater', 'func': 'search_links', 'foldername': 'Search'})
    li = xbmcgui.ListItem('Search', iconImage=imageDir + 'search.jpg')
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url,
                                listitem=li, isFolder=True)

    url = build_url({'mode': 'bangyoulater', 'func': 'category_directorys', 'foldername': 'Categories'})
...
def category_directorys():
    top_url = 'http://www.bangyoulater.com'
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    category_links = re.compile('f="(\/video\/[^"]+)">([^<]+)').findall(url_content)

    if category_links:
        for cic in range(0, len(category_links)):
            url = build_url({'mode': 'bangyoulater', 'func': 'video_links', 'foldername': category_links[cic][0], 'pagenum': '0'})
...
def video_links(foldername, pagenum):
    ...
    if re.compile('(/?search)').findall(foldername):
        top_url = 'http://www.bangyoulater.com' + foldername[:20] + pagenum + foldername[20:]
    else:
        top_url = 'http://www.bangyoulater.com' + foldername + pagenum + '/'

    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return
    
    porn_links = re.compile('href="([^"]+)" title="([^"]+).+src="([^"]+)').findall(url_content)
    no_page = re.compile('(No result)').findall(url_content)

    if len(porn_links) >= 20:
        next_page = True
    else:
        next_page = False
    ...
    if no_page:
        ...
        for cic in range(0, len(porn_links)):
            url = build_url({'mode': 'bangyoulater', 'func': 'find_videourls', 'foldername': porn_links[cic][0], 'title': porn_links[cic][1], 'folderimage': porn_links[cic][2]})
        ...
        if next_page:
            url = build_url({'mode': 'bangyoulater', 'func': 'video_links', 'foldername': foldername, 'pagenum': pagenum})
...
def search_links():
    ...
    top_url = '/porn-search/videos//?search=' + urllib.quote(search_text, '')
...
def find_videourls(foldername, foldertitle, folderimage, isdownload):
    top_url = 'http://www.bangyoulater.com' + foldername
            
    url_content = find_read_error(top_url)
    if url_content == 'HIBA':
        return

    direct_url = re.compile('_videoUrl[^\']+\'([^\']+)').findall(url_content)
